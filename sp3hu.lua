...
local setclipboard = setclipboard or toclipboard or set_clipboard or (Clipboard and Clipboard.set) or function(...)
    return ErrorPrompt("试图设置剪贴板: "..(...),true)
end
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "KOLK",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
local ImageLabel = Create("ImageLabel",{Parent = CloseButton,BackgroundColor3 = Color3.new(255, 155, 155),BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 5),Size = UDim2.new(0, 9, 0, 9),Image = "http://www.roblox.com/asset/?id=5597086202"})
...
local MinimizeButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(125.145098, 0.141176, 0.14902),BorderSizePixel = 0,Position = UDim2.new(1, -57, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
...
local ToolTip = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Color3.fromRGB(155, 155, 155),BackgroundTransparency = 0.1,BorderColor3 = Color3.new(1, 1, 1),Size = UDim2.new(0, 200, 0, 50),ZIndex = 3,Visible = false})
...
function newRemote(type, data)
    ...
    local log = {
        Name = remote.name,
        Function = data.infofunc or "--功能信息被禁用",
        Remote = remote,
        DebugId = data.id,
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- SIMPLESPY阻止此远程设备向服务器发射\n\n" .. log.GenScript
        end
...
function genScript(remote, args)
    ...
    if #args > 0 then
        xpcall(function()
            ...
            gen ..= "-- 出现了一个错误:\n--"..err.."\n-- TableToString失败！恢复到传统功能(结果可能会有所不同)\n本地参数 = {"
            xpcall(function()
                for i, v in next, args do
...
ufunctions = {
    ...
    RBXScriptSignal = function(u) -- The server doesnt recive this
        return "RBXScriptSignal --[[不支持RBXScriptSignal]]"
    end,
    RBXScriptConnection = function(u) -- The server doesnt recive this
        return "RBXScriptConnection --[[不支持RBXScriptConnection]]"
    end,
}
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do -- iterates over table
        ...
        if size > (getgenv().SimpleSpyMaxTableSize or 1000) then
            s = s .. "\n" .. string.rep(" ", l) .. "-- 到最大表格的大小请更改'getgenv().SimpleSpyMaxTableSize' "
            break
        end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        if not RunService:IsClient() then
            error("SimpleSpy无法在服务器上运行!")
        end
        getgenv().SimpleSpyShutdown = shutdown
        onToggleButtonClick()
        if not hookmetamethod then
            ErrorPrompt("由于您的执行器不支持hookmetamethod，Simple Spy V3将无法充分发挥其功能.",true)
        end
        codebox = Highlight.new(CodeBox)
        logthread(spawn(function()
            local suc,err = pcall(game.HttpGet,game,"https://raw.githubusercontent.com/Finaloutcome/plz/refs/heads/main/update.txt")
...
newButton(
    "复制交流群号",
    function() return "复制交流群的群号" end,
    function()
        setclipboard("993659706")
        TextLabel.Text = "复制成功!"
    end
)
...
newButton(
    "复制远程",
    function() return "点按以复制遥控器的路径" end,
    function()
...
--- 反编译启动遥控器的脚本，并将其放入代码框中
newButton("功能信息",function() return "单击查看调用函数信息" end,
function()
    ...
    if func then
        ...
        if typeoffunc ~= 'string' then
            ...
            codebox:setRaw("--[[正在将表格转换为字符串，请稍候]]")
            selected.Function = v2v({functionInfo = info})
        end
        codebox:setRaw("-- 调用功能信息\n-- 由SimpleSpy V3序列化程序生成\n\n"..selected.Function)
        TextLabel.Text = "搞定了。SimpleSpy V3生成的函数信息 串行器."
    else
        TextLabel.Text = "错误！找不到选定的函数."
    end
end)
...
newButton(
    "排除 (n)",
    function() return "单击以排除所有具有此名称的远程.\n排除遥控器会使SimpleSpy忽略它，但它将继续可用." end,
    function()
        if selected then
...
newButton(
    "排除 (i)",
    function() return "按一下以停止这个遥控器发射.\n阻止远程不会将其从SimpleSpy日志中删除，但不会继续触发服务器." end,
    function()
        if selected then
...
--- 防止所有与选定的具有相同名称的远程控件触发。从RemoteSpy远程记录(仍然记录)
newButton("排除 (n)",function()
    return "按一下以停止使用这个名称的遥控器.\n阻止远程不会将它从SimpleSpy日志中删除，但不会继续触发服务器。" end,
    function()
        if selected then
    ...
    function()
        ...
        if decompile then
            if selected and selected.Source then
                ...
                codebox:setRaw(DecompiledScripts[Source] or "--找不到来源")
                TextLabel.Text = "完成的!"
            else
                TextLabel.Text = "找不到源!"
            end
...
newButton(
    "禁用信息",
    function() return string.format("[%s] 切换功能信息(因为它会导致一些游戏延迟)", configs.funcEnabled and "ENABLED" or "DISABLED") end,
    function()
        configs.funcEnabled = not configs.funcEnabled
        TextLabel.Text = string.format("[%s] 切换功能信息(因为它会导致一些游戏延迟)", configs.funcEnabled and "ENABLED" or "DISABLED")
    end
)

newButton(
    "自动拦截",
    function() return string.format("[%s] [BETA] 智能检测并从日志中排除垃圾远程呼叫", configs.autoblock and "ENABLED" or "DISABLED") end,
    function()
        configs.autoblock = not configs.autoblock
        TextLabel.Text = string.format("[%s] [BETA] 智能检测并从日志中排除垃圾远程呼叫", configs.autoblock and "ENABLED" or "DISABLED")
        history = {}
        excluding = {}
    end
)

newButton("日志检查调用",function()
    return ("[%s] 记录客户端触发的远程操作"):format(configs.logcheckcaller and "ENABLED" or "DISABLED")
end,
function()
    configs.logcheckcaller = not configs.logcheckcaller
    TextLabel.Text = ("[%s] 记录客户端触发的远程操作"):format(configs.logcheckcaller and "ENABLED" or "DISABLED")
end)
...
newButton("高级信息",function()
    return ("[%s] 显示更多远程信息"):format(configs.advancedinfo and "ENABLED" or "DISABLED")
end,
function()
...
if configs.supersecretdevtoggle then
    newButton("使用spy 2.2",function()
        return "Load's Simple Spy V2.2"
    end,
    function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/exxtremestuffs/SimpleSpySource/master/SimpleSpy.lua"))()
    end)
    newButton("加载V3",function()
        return "当前版本已经是v3！"
    end,
    function()
        return nil
